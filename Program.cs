...
namespace test_task
{
    ...
    static class JavaTokenizer
    {
        ...
        static TextParser<Unit> JavaNumberToken { get; } =
            from sign in Character.EqualTo('-').OptionalOrDefault()
            from first in Character.Digit
            from rest in Character.Digit.Or(Character.In('.', 'e', 'E', '+', '-')).IgnoreMany()
            select Unit.Value;

        static TextParser<Unit> JavaIntTypeToken { get; } =
            from content in Span.EqualTo("int")
            select Unit.Value;

        static TextParser<Unit> JavaSinkToken { get; } =
            from content in Span.EqualTo("System.out.println")
            select Unit.Value;

        static TextParser<Unit> JavaIfToken { get; } =
            from content in Span.EqualTo("if")
            select Unit.Value;

        static TextParser<Unit> JavaOtherToken { get; } =
            from content in Span.Regex("\\w+")
            select Unit.Value;
        ...
        public static Tokenizer<JavaToken> Instance { get; } = 
            new TokenizerBuilder<JavaToken>()
                .Ignore(Span.WhiteSpace)
                .Match(Character.EqualTo('{'), JavaToken.LBracket)
                .Match(Character.EqualTo('}'), JavaToken.RBracket)
                .Match(Character.EqualTo(':'), JavaToken.Semicolon)
                .Match(Character.EqualTo(','), JavaToken.Comma)
                .Match(Character.EqualTo('['), JavaToken.LSquareBracket)
                .Match(Character.EqualTo(']'), JavaToken.RSquareBracket)
                .Match(Character.EqualTo('('), JavaToken.LParentheses)
                .Match(Character.EqualTo(')'), JavaToken.RParentheses)
                .Match(Character.EqualTo('='), JavaToken.Equals)
                .Match(JavaIntTypeToken,       JavaToken.IntType, requireDelimiters: true)
                .Match(JavaIfToken,            JavaToken.If, requireDelimiters: true)
                .Match(JavaSinkToken,          JavaToken.Sink, requireDelimiters: true)
                .Match(JavaNumberToken,        JavaToken.Number, requireDelimiters: true)
                .Match(JavaOtherToken,         JavaToken.Other, requireDelimiters: true)
                .Match(Identifier.CStyle,      JavaToken.Identifier, requireDelimiters: true)
                .Build();
    }
    ...
    // `JavaTextParsers` class contains two standalone character-driven parsers
    // that can handle the (non-trivial) string and number formats from
    // the JSON spec.
    static class JavaTextParsers
    {
    ...
    static class JsonParser
    {
        ...
        static TokenListParser<JavaToken, object> JsonString { get; } =
            Token.EqualTo(JavaToken.Other)
                .Apply(JavaTextParsers.String)
                .Select(s => (object)s);
        
        static TokenListParser<JavaToken, object> JsonNumber { get; } =
            Token.EqualTo(JavaToken.Number)
                .Apply(JavaTextParsers.Number)
                .Select(n => (object)n);
        ...
        static TokenListParser<JavaToken, object> JsonObject { get; } =
            from begin in Token.EqualTo(JavaToken.LBracket)
            from properties in JsonString
                .Named("property name")
                .Then(name => Token.EqualTo(JavaToken.Colon)
                    .IgnoreThen(Parse.Ref(() => JsonValue)
                    .Select(value => KeyValuePair.Create((string)name, value))))    
                .ManyDelimitedBy(Token.EqualTo(JavaToken.Comma),
                    end: Token.EqualTo(JavaToken.RBracket))
        ...
        static TokenListParser<JavaToken, object> JsonArray { get; } =
            from begin in Token.EqualTo(JavaToken.LSquareBracket)
            from values in Parse.Ref(() => JsonValue)
                .ManyDelimitedBy(Token.EqualTo(JavaToken.Comma),
                    end: Token.EqualTo(JavaToken.RSquareBracket))
            select (object)values;

        static TokenListParser<JavaToken, object> JsonTrue { get; } =
            Token.EqualToValue(JavaToken.Identifier, "true").Value((object)true);
        
        static TokenListParser<JavaToken, object> JsonFalse { get; } =
            Token.EqualToValue(JavaToken.Identifier, "false").Value((object)false);    

        static TokenListParser<JavaToken, object> JsonNull { get; } =
            Token.EqualToValue(JavaToken.Identifier, "null").Value((object)null);

        static TokenListParser<JavaToken, object> JsonValue { get; } =
            JsonString
                .Or(JsonNumber)
                .Or(JsonObject)
                .Or(JsonArray)
        ...
        static TokenListParser<JavaToken, object> JsonDocument { get; } = JsonValue.AtEnd();
        ...
        {
            var tokens = JavaTokenizer.Instance.TryTokenize(json);
            if (!tokens.HasValue)
            {
                value = null;
